...
/* Public functions ----------------------------------------------------------*/
/* Public variables ----------------------------------------------------------*/
ITStatus rxActive = RESET;
uint8_t UART_data_count = 0;
uint8_t UART_data_index = 0;
uint8_t UART_data_start = 0;
uint8_t UART_buffer[RADIO_MAX_PACKET_LENGTH];
/* ---------------------------------------------------------------------------*/
...
{
    /*
     * Interrupt received when a SYNCWORD is received by the 4463 i.e. data is
     * being received.
     */
    rxActive = SET;
}
...
{
    /*
     * Received data into a cyclic buffer and have the TX routine pull data
     * from this.  Note that we cannot guarentee no lost data so the application
     * sending the data needs to implement hand-shaking to ensure consistent
     * results.
     */
		// !!PDS: Debugging
		// uint8_t status = 0;
    UART_buffer[UART_data_index++] = UART1_ReceiveData8();
		// status = UART1->SR;
    // UART1_SendData8(0x31);
		UART_data_count++;
    if (UART_data_index > RADIO_MAX_PACKET_LENGTH)
    {
        UART_data_index = 0;
    }
    return;
