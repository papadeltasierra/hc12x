...
void main(void)
{
    ...
    /* Start the radio --------------------------------------------*/
    vRadio_StartRX(pFixedRadioConfiguration->Radio_ChannelNumber);
...
void Pollhandler()
{
    ...
    if (
        UART1_GetITStatus(UART1_IT_RXNE))
    {
        /*
         * UART is receiving data; receive the data into the TX buffer and then
         * attempt to transmit.  We read data until either the buffer is full or
         * there is no more data to receive.
         *
         * TODO: Should we be waiting on a short timer in case the sender is slower
         *       than we are?  Might want to test this somehow.
         */
        len = 0;
        ptr = &txRadioPacket[0];

        if (FixedRadioConfiguration.Radio_PacketLength != 0)
        {
            /**
             * Fixed length so have to wait for all the bytes to be ready.
             */
            len = FixedRadioConfiguration.Radio_PacketLength;
            while (len--)
            {
        ...
        else
        {
             ...
             */
            len = RADIO_MAX_PACKET_LENGTH;
            while ((len--) && UART1_GetFlagStatus(UART1_FLAG_RXNE))
            {
                *ptr++ = UART1_ReceiveData8();
            }
            len = RADIO_MAX_PACKET_LENGTH - len;
        }
        vRadio_StartTx(0, len, txRadioPacket);
    }
